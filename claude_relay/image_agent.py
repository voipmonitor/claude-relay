...
def strip_and_cache_images(body: dict, session_id: str, image_cache: ImageCache) -> dict:
    """Replace images with [Image #N] placeholders, cache originals, inject analyzeImage tool.

    Mutates and returns the body dict.

    Clears the session cache before repopulating to ensure fresh images each turn
    and avoid stale entries from previous requests.
    """
    # Clear session cache before repopulating (stateless request processing)
    image_cache._sessions.pop(session_id, None)
    ...
    for msg in body.get("messages", []):
        ...
        for block in content:
            if block.get("type") == "image" and block.get("source"):
                ...
                image_cache.store(session_id, key, block["source"])
                new_content.append({
                    "type": "text",
                ...
                for item in block["content"]:
                    if isinstance(item, dict) and item.get("type") == "image" and item.get("source"):
                        key = f"{session_id}_Image#{counter}"
                        image_cache.store(session_id, key, item["source"])
                        new_items.append({
                            "type": "text",
                            "text": f'[Image #{counter}] — YOU CANNOT SEE THIS IMAGE. Call analyzeImage(imageId=["{counter}"]) to view it.',
                        })
...
async def image_agent_stream(
    ...
    for img_id in image_ids:
        ...
        source = image_cache.get(session_id, key)
        if source:

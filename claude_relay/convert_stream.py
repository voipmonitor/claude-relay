...
async def convert_openai_stream_to_anthropic(
    ...
    open_tool_blocks: set[int] = set()  # anthropic block indices for open tool_use blocks
    open_blocks: set[int] = set()  # anthropic block indices for any open content block (text/thinking/tool_use)
    ...
    async for event in sse_events:
        ...
        if tool_calls:
            ...
            for tc in tool_calls:
                ...
                if tc_index not in tool_call_blocks:
                    ...
                    tool_id = tc.get("id", "")
                    if tool_name:
                        tool_names_by_index[tc_index] = tool_name
                ...
                if func.get("name"):
                    tool_names_by_index[tc_index] = func["name"]
                if isinstance(args_chunk, str):
                    tool_arg_buffers[tc_index] = tool_arg_buffers.get(tc_index, "") + args_chunk
                if args_chunk:
        ...
        if finish_reason:
            ...
            if finish_reason == "tool_calls" and not tool_call_blocks:
                log.error(
                    "%sconvert_stream: backend ended with finish_reason=tool_calls but no delta.tool_calls were seen; delta_keys=%s content_preview=%r",
                    _r,
                    sorted(delta.keys()),
                    str(delta.get("content", ""))[:200],
                )
    ...
    # Close any block still open at end-of-stream (text, thinking, or parallel tool_use).
    # sorted(list(...)) because _stop_block mutates open_blocks via discard.
    for idx in sorted(list(open_blocks)):
    ...
    elif pending_stop_reason == "tool_use":
        for tc_index, args in sorted(tool_arg_buffers.items()):
            if not args:
                continue
            try:
                json.loads(args)
                ...
                log.error(
                    "%sconvert_stream: invalid JSON arguments for tool index=%s name=%s: %s args_preview=%r",
                    _r,
                    tc_index,
                    tool_names_by_index.get(tc_index, ""),
                    e,
                    args[:300],
                )
...
async def _test_parallel_tool_calls():
    """Test that parallel tool calls all get closed."""
    ...
    async def mock_events():
        yield MockEvent('{"model":"test"}')
        yield MockEvent('{"choices":[{"delta":{"tool_calls":[{"index":0,"id":"t1","function":{"name":"foo","arguments":"{"}}]},"finish_reason":null}]}')
        yield MockEvent('{"choices":[{"delta":{"tool_calls":[{"index":1,"id":"t2","function":{"name":"bar","arguments":"{"}}]},"finish_reason":null}]}')
        yield MockEvent('{"choices":[{"delta":{},"finish_reason":"tool_calls"}]}')
        yield MockEvent('[DONE]')

    result = StreamResult()
    chunks = [c async for c in convert_openai_stream_to_anthropic(mock_events(), result=result)]
    output = b"".join(chunks).decode()
    stop_count = output.count("event: content_block_stop")
    assert stop_count == 2, f"Expected 2 stops, got: {stop_count}"
    print("TEST PASSED: parallel tool calls close all blocks")
